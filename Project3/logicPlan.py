...
def sentence1() -> Expr:
    ...
    b=Expr('B')
    c=Expr('C')
    #Performing steps as mentioned on course website.
    first=a|b
    second=~a % (~b|c)
    third=disjoin(~ a, ~b, c)
...
def sentence3() -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    #Creating literals
    p=PropSymbolExpr('PacmanAlive_1')
    a=PropSymbolExpr('PacmanAlive_0')
    b=PropSymbolExpr('PacmanBorn_0')
    k=PropSymbolExpr('PacmanKilled_0')
    #Performing steps as mentioned on course website.
    s1=p%((a&~k)|(~a&b))
    s2=~(a & b)
    s3=b
...
def findModelCheck() -> Dict[Any, bool]:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    #Returning dummyclass to allow lower case 'a'.
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    #Because if KB entails x then we need to prove KB and ~x is flase.
    proof= not findModel(premise & ~conclusion)
    return proof
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def atLeastOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    #or of literals as its true if any one of literal is true
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # this evaluates to ~(A1 & A2) & ~(A1 &A3)...~(A1 & An )....~(An-1 &An)
    coll=[]
    for i in range(len(literals)-1):
        for j in range(i+1,len(literals)):
            ex=(~literals[i] | ~literals[j])
            coll.append(ex)
    return conjoin(coll)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def exactlyOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    #If clause has atleast and atmost literal true then we have exactly 1 literal true.
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    #As mentioned in comments above current state double implies possible causes.
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    pacman_not_on_wall=[]
    #Pacman should not be on wall.
    for each in all_coords:
        pacman_not_on_wall.append(PropSymbolExpr(wall_str,each[0],each[1]) >> ~PropSymbolExpr(pacman_str, each[0], each[1],time=t))
    #pacnman at only 1 square at any time.
    at_exactly_one=exactlyOne([PropSymbolExpr(pacman_str,each[0],each[1],time=t) for each in non_outer_wall_coords])
    #Take any one step at any time.
    successor_actions=exactlyOne([PropSymbolExpr(direction,time=t) for direction in DIRECTIONS])
    pacphysics_sentences=[conjoin(pacman_not_on_wall),at_exactly_one,successor_actions]
    #adding result of sensor model.
    if(sensorModel!=None):
        pacphysics_sentences.append(sensorModel(t, non_outer_wall_coords))
    #adding result of successorAxioms model.
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    "*** BEGIN YOUR CODE HERE ***"
    #As described on course website adding following expressions to kb 
    ...
    return findModel(conjoin(conjoin(KB),PropSymbolExpr(pacman_str,x1_y1[0],x1_y1[1],time=1))) ,findModel(conjoin(conjoin(KB),~PropSymbolExpr(pacman_str,x1_y1[0],x1_y1[1],time=1))) 
...
def positionLogicPlan(problem) -> List:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    #As described on course website adding following expression to kb.
    KB.append(PropSymbolExpr(pacman_str,x0,y0,time=0))
    #Implentation of algorithm as mentioned on course website.
    for t in range(0,51):
        print(t)
        # pacman can only be at one position at a time and it should not be a wall.
        KB.append(exactlyOne([PropSymbolExpr(pacman_str,each[0],each[1],time=t) for each in non_wall_coords]))
        #Check if model satisfies.
        is_satisfy=findModel(conjoin(conjoin(KB),PropSymbolExpr(pacman_str,xg,yg,time=t)))
        #if yes return steps.
        if is_satisfy:
            return extractActionSequence(is_satisfy,actions)
        else:
            #else add to kb that pacman will take only one direction.
            KB.append(exactlyOne([PropSymbolExpr(i,time=t) for i in actions]))
            #add all possibel non wall coordinates where pacman can be at time t+1
            for i in non_wall_coords:
                KB.append(pacmanSuccessorAxiomSingle(i[0], i[1], time=t+1,walls_grid=walls_grid))
